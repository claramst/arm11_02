...
INSTR_TOKENS *tokenize(char *instrLine, int address, Map *symbolTable) {
    ...
    for (int i = 0; i < MAX; i++) {
        switch (instrLine[i]) {
            ...
            case '[':
                tokens->symbols[index] = OPEN;
            case ']':
                tokens->symbols[index] = CLOSE;
            case '+':
                tokens->symbols[index] = PLUS;
            case '-':
                tokens->symbols[index] = MINUS;
            case '#':
                tokens->symbols[index] = HASHTAG;
            case '=':
                tokens->symbols[index] = EQUAL;
        }
    }
    /*
