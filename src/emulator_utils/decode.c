...
DECODED_INSTR decode(INSTRUCTION instr, MACHINE_STATE state) {
  ...
  switch (type) {
	case MULTIPLY:decoded.A = getBit(instr, 21);
	...
	case PROCESSING:decoded.opcode = getBits(instr, 4, 21);
	  ...
	  break;
	case TRANSFER:decoded.I = getBit(instr, 25);
	  ...
	  if (decoded.I) {
		...
		switch (shiftType) {
		  ...
		  case 1:decoded.offset = decoded.rm >> shiftAmount;
			break;
		  case 2:decoded.offset = arithmeticRight(decoded.rm, shiftAmount);
			break;
		  case 3:decoded.offset = rotateRight(decoded.rm, shiftAmount);
			break;
		}
	  } else {
