...
void info(Editor *state) {
  if (state->noOfTokens == 1) {
	...
	} else if (SAME(state->tokens[1], "continue")) {
	  printf("Runs through the program until it hits the next breakpoint.\n");
	} else if (SAME(state->tokens[1], "break")) {
	  printf("Sets a breakpoint at the desired line.\n");
	} else if (SAME(state->tokens[1], "disable")) {
